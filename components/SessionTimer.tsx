...
function computeState(): TimerState {
  ...
  const MID = 9 * 3600 + 45 * 60   // OR locks at 9:45 (15-min opening range)
  const CLOSE = 11 * 3600 + 30 * 60 // session ends 11:30 — matches engine + alerts
  const WARN = 9 * 3600
  ...
  if (!isWeekday) {
    ...
    const t = pad(secs)
    return { phase: 'weekend', label: 'WEEKEND', sublabel: 'Next session: Monday 9:30 AM ET · 6:30 PM Dubai', hours: t.h, minutes: t.m, seconds: t.s, color: 'var(--text-secondary)', bgColor: 'var(--surface)', borderColor: 'var(--border)', nyTime, dubaiTime, dayLabel: dayNames[day], progressPct: 0 }
  }
  ...
  if (totalSecs >= CLOSE) {
    ...
    return { phase: 'closed', label: 'SESSION CLOSED', sublabel: `Next: ${nextDay} 9:30 AM ET · 6:30 PM Dubai`, hours: t.h, minutes: t.m, seconds: t.s, color: 'var(--text-dim)', bgColor: 'var(--surface)', borderColor: 'var(--border)', nyTime, dubaiTime, dayLabel: dayNames[day], progressPct: 100 }
  }
  ...
  if (totalSecs >= OPEN && totalSecs < MID) {
    ...
    return { phase: 'orb_window', label: '🔴 LIVE — OPENING RANGE BUILDING', sublabel: 'OR closes at 9:45 AM ET (6:45 PM Dubai) — wait for the range to form', hours: 0, minutes: t.m, seconds: t.s, color: 'var(--red)', bgColor: 'var(--red-bg)', borderColor: 'var(--red-border)', nyTime, dubaiTime, dayLabel: dayNames[day], progressPct: pct }
  }
  ...
  if (totalSecs >= MID && totalSecs < CLOSE) {
    ...
    return { phase: 'post_orb', label: '🟢 LIVE — OR COMPLETE · TRADE WINDOW OPEN', sublabel: 'Session closes at 11:30 AM ET (7:30 PM Dubai)', hours: 0, minutes: t.m, seconds: t.s, color: 'var(--green)', bgColor: 'var(--green-bg)', borderColor: 'var(--green-border)', nyTime, dubaiTime, dayLabel: dayNames[day], progressPct: pct }
  }
  ...
  if (totalSecs >= WARN && totalSecs < OPEN) {
    ...
    return { phase: 'approaching', label: '⚡ SESSION STARTING SOON', sublabel: 'NY Open 9:30 AM ET · Dubai 6:30 PM · Prepare your setup now', hours: 0, minutes: t.m, seconds: t.s, color: 'var(--yellow)', bgColor: 'var(--yellow-bg)', borderColor: 'var(--yellow-border)', nyTime, dubaiTime, dayLabel: dayNames[day], progressPct: pct }
  }

  const secs = OPEN - totalSecs
  const t = pad(secs)
  return { phase: 'pre_session', label: 'NEXT SESSION', sublabel: 'NY Open 9:30 AM ET · Dubai 6:30 PM · 7:30 PM close', hours: t.h, minutes: t.m, seconds: t.s, color: 'var(--blue)', bgColor: 'var(--surface)', borderColor: 'var(--border)', nyTime, dubaiTime, dayLabel: dayNames[day], progressPct: 0 }
}
...
export default function SessionTimer() {
  ...
  return (
    <div style={{
      background: s.bgColor, border: `1px solid ${s.borderColor}`, borderRadius: '10px',
      ...
      <div style={{ marginTop: '14px' }}>
        ...
        <div style={{ position: 'relative', height: '6px', background: 'var(--border)', borderRadius: '3px', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'absolute', left: '50%', top: 0, bottom: 0, width: '50%', background: s.phase === 'post_orb' ? 'var(--green)' : 'var(--border-bright)', opacity: s.phase === 'post_orb' ? 1 : 0.4, transition: 'background 0.3s' }} />
          {isLive && (
            <div style={{ position: 'absolute', left: s.phase === 'orb_window' ? `${(s.progressPct / 100) * 50}%` : `${50 + (s.progressPct / 100) * 50}%`, top: '50%', transform: 'translate(-50%, -50%)', width: '10px', height: '10px', borderRadius: '50%', background: s.color, boxShadow: `0 0 8px ${s.color}`, transition: 'left 1s linear' }} />
          )}
...
// Short labels so it doesn't overflow the navbar
export function SessionTimerNavbar() {
  ...
  // Short labels for navbar — the full labels go in the page-level SessionTimer
  const navLabel =
    s.phase === 'orb_window'  ? 'OR BUILDING' :
    s.phase === 'post_orb'    ? 'TRADE WINDOW' :
    s.phase === 'approaching' ? 'STARTING SOON' :
    s.phase === 'pre_session' ? 'NEXT SESSION' :
    s.phase === 'closed'      ? 'CLOSED' :
    s.phase === 'weekend'     ? 'WEEKEND' : ''
