...
//   Daily loss limit        : $2,000  (shown separately)
//   Profit target           : $6,000

import { useEffect, useRef, useState } from 'react'

const TRAILING_LIMIT   = 3_000   // Topstep $100K trailing drawdown
const DAILY_LIMIT      = 2_000   // daily loss limit
const PROFIT_TARGET    = 6_000   // profit target
const START_BALANCE    = 100_000 // initial account balance
const DANGER_THRESHOLD = 500     // go red when this close to floor
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
          <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '10px', color: 'var(--text-dim)' }}>
            DRAWDOWN USED — ${Math.round(remaining).toLocaleString()} of $3,000
          </span>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px', fontFamily: 'IBM Plex Mono, monospace', fontSize: '9px', color: 'var(--text-dim)' }}>
          <span>$0 used</span>
          <span style={{ color: 'var(--yellow)' }}>⚠ $2,500</span>
          <span style={{ color: 'var(--red)' }}>✕ $3,000 MAX</span>
        </div>
      </div>
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
          <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '10px', color: 'var(--text-dim)' }}>
            PROFIT TARGET — ${Math.max(0, profitPnL).toLocaleString()} of $6,000
          </span>
