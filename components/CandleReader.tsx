...
function detectPattern(candles: Candle[], idx: number): PatternResult | null {
  ...
  // Shooting Star
  if (upperWickRatio > 0.55 && lowerWickRatio < 0.2 && bodyRatio > 0.05 && !isBullish) return { name: 'Shooting Star', emoji: '⭐', signal: 'bearish', strength: 71,
  ...
  if (prev && isBearish && prev.close > prev.open) {
    if (body > Math.abs(prev.close - prev.open) * 1.0 && bodyRatio > 0.4) return { name: 'Bearish Engulfing', emoji: '📉', signal: 'bearish', strength: 82,
      meaning: 'The current red candle completely swallows the previous green candle body — sellers overwhelmed buyers in a single decisive move. High-conviction reversal signal.',
      orbContext: 'OR High rejection confirmed. The false breakout trap has materialized. Exit longs immediately. This is the short entry candle.',
...
export default function CandleReader({ orHigh, orLow }: { orHigh?: number | null; orLow?: number | null }) {
  ...
  const getIntervalStart = useCallback((ts: number) => {
    return Math.floor(ts / (timeframe * 60 * 1000)) * (timeframe * 60 * 1000)
  }, [timeframe])
  ...
  // All candles including live
  const allCandles = [...candles, ...(liveCandle ? [liveCandle] : [])]
  const visibleCandles = allCandles.slice(-14) // show last 14

  // Chart dimensions
  const CHART_H = 220
  const CHART_W = 680
  const PAD_L = 8; const PAD_R = 48; const PAD_T = 20; const PAD_B = 24
  const chartInnerW = CHART_W - PAD_L - PAD_R
  ...
  const toY = (p: number) => PAD_T + chartInnerH - ((p - priceMin) / priceRange) * chartInnerH
  const slotW = chartInnerW / 14
  const bodyW = Math.max(slotW * 0.55, 7)
  ...
  const selectedPattern = selectedIdx !== null && selectedIdx < candles.length
    ? detectPattern(candles, selectedIdx)
    : null
    ...
    <div className="card" style={{ marginBottom: '20px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
          ...
          {lastPrice && orHigh && orLow && (
            <span style={{ fontSize: '9px', padding: '2px 6px', borderRadius: '4px', fontFamily: 'IBM Plex Mono, monospace',
              background: lastPrice > orHigh ? 'rgba(34,197,94,0.12)' : lastPrice < orLow ? 'rgba(239,68,68,0.12)' : 'rgba(107,114,128,0.12)',
              color: lastPrice > orHigh ? '#22c55e' : lastPrice < orLow ? '#ef4444' : '#9ca3af' }}>
              {lastPrice > orHigh ? 'ABOVE OR HIGH' : lastPrice < orLow ? 'BELOW OR LOW' : 'INSIDE OR'}
            </span>
          )}
          <span style={{ fontSize: '9px', color: 'var(--text-dim)', fontFamily: 'IBM Plex Mono, monospace' }}>Click any candle to read it</span>
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', marginBottom: '12px' }}>
        <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '28px', fontWeight: '700', letterSpacing: '-0.02em', transition: 'color 0.2s',
          color: priceDir === 'up' ? '#22c55e' : priceDir === 'down' ? '#ef4444' : 'var(--text-primary)' }}>
          {lastPrice ? lastPrice.toFixed(2) : '—'}
        </span>
        {priceDir && <span style={{ fontSize: '18px', color: priceDir === 'up' ? '#22c55e' : '#ef4444' }}>{priceDir === 'up' ? '▲' : '▼'}</span>}
        {liveCandle && (
          <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '12px', marginLeft: '4px',
      ...
      {/* ── Candle Chart ── */}
      <div style={{ background: 'var(--surface)', borderRadius: '10px', padding: '8px', marginBottom: '14px', overflowX: 'auto' }}>
        {visibleCandles.length === 0 ? (
          <div style={{ height: `${CHART_H}px`, display: 'flex', alignItems: 'center', justifyContent: 'center',
            color: 'var(--text-dim)', fontFamily: 'IBM Plex Mono, monospace', fontSize: '12px' }}>
            Waiting for first tick...
          </div>
        ) : (
          <svg width={CHART_W} height={CHART_H} style={{ display: 'block', minWidth: `${CHART_W}px` }}>
            {/* Grid lines */}
            {[0, 0.25, 0.5, 0.75, 1].map(pct => {
              const y = PAD_T + chartInnerH * pct
              const price = priceMax - pct * priceRange
              return (
                <g key={pct}>
                  <line x1={PAD_L} y1={y} x2={CHART_W - PAD_R} y2={y} stroke="rgba(255,255,255,0.04)" strokeWidth="1" />
                  <text x={CHART_W - PAD_R + 4} y={y + 4} fill="#4b5563" fontSize="8" fontFamily="monospace">{price.toFixed(0)}</text>
                </g>
              )
            })}
            ...
            {orHigh && orHigh > 0 && orHigh >= priceMin && orHigh <= priceMax && (
              <>
                <line x1={PAD_L} y1={toY(orHigh)} x2={CHART_W - PAD_R} y2={toY(orHigh)} stroke="#22c55e" strokeWidth="1" strokeDasharray="5,4" opacity={0.6} />
                <text x={CHART_W - PAD_R + 4} y={toY(orHigh) + 4} fill="#22c55e" fontSize="8" fontFamily="monospace">OR H</text>
              </>
            )}
            ...
            {orLow && orLow > 0 && orLow >= priceMin && orLow <= priceMax && (
              <>
                <line x1={PAD_L} y1={toY(orLow)} x2={CHART_W - PAD_R} y2={toY(orLow)} stroke="#ef4444" strokeWidth="1" strokeDasharray="5,4" opacity={0.6} />
                <text x={CHART_W - PAD_R + 4} y={toY(orLow) + 4} fill="#ef4444" fontSize="8" fontFamily="monospace">OR L</text>
              </>
            )}
            ...
            {visibleCandles.map((c, i) => {
              ...
              const baseColor = bull ? '#22c55e' : '#ef4444'
              ...
              // Pattern label for completed candles
              const pat = !isLive ? detectPattern(candles, globalIdx) : null
              const patShort = pat ? pat.emoji : null
              ...
              return (
                <g key={i} style={{ cursor: isLive ? 'default' : 'pointer' }}
                  ...
                  {/* Selection highlight */}
                  {isSel && (
                    <rect x={PAD_L + i * slotW + 1} y={PAD_T} width={slotW - 2} height={chartInnerH}
                      fill="rgba(255,255,255,0.04)" rx="2" />
                  )}

                  {/* Wick */}
                  <line x1={cx} y1={wickTop} x2={cx} y2={wickBot}
                    stroke={isLive ? '#facc15' : baseColor}
                    strokeWidth={isLive ? 1.5 : 1.5}
                    opacity={isLive ? 0.8 : 1} />

                  {/* Body */}
                  <rect x={cx - bodyW / 2} y={bodyTop} width={bodyW} height={bodyH}
                    fill={isLive ? '#facc1540' : (bull ? '#22c55e' : '#ef4444')}
                    stroke={isLive ? '#facc15' : baseColor}
                    strokeWidth={isSel ? 1.5 : 1}
                    rx="1.5"
                    opacity={isLive ? 0.9 : 1} />

                  {/* LIVE label on forming candle */}
                  {isLive && (
                    <text x={cx} y={PAD_T - 6} fill="#facc15" fontSize="7" textAnchor="middle" fontFamily="monospace">LIVE</text>
                  )}

                  {/* Pattern emoji label on completed candles */}
                  {patShort && !isLive && (
                    <text x={cx} y={bull ? bodyTop - 5 : bodyBot + 12} fill={isSel ? '#fff' : '#9ca3af'}
                      fontSize="9" textAnchor="middle" style={{ userSelect: 'none' }}>{patShort}</text>
                  )}

                  {/* Time label at bottom */}
                  {i % 3 === 0 && (
                    <text x={cx} y={CHART_H - 6} fill="#374151" fontSize="7" textAnchor="middle" fontFamily="monospace">
      ...
      {selectedCandle && selectedIdx !== null && !selectedIsLive ? (
        selectedPattern ? (() => {
          const sig = sigColors[selectedPattern.signal]
          return (
            <div style={{ background: sig.bg, border: `1px solid ${sig.border}`, borderRadius: '12px', padding: '16px 18px' }}>
              {/* Pattern name row */}
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <span style={{ fontSize: '26px' }}>{selectedPattern.emoji}</span>
                  <div>
                    <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '14px', fontWeight: '700', color: sig.color }}>{selectedPattern.name}</div>
                    <div style={{ fontSize: '10px', color: 'var(--text-dim)', fontFamily: 'IBM Plex Mono, monospace', marginTop: '2px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '6px' }}>
                  <span style={{ padding: '3px 10px', borderRadius: '4px', background: `${sig.color}20`, color: sig.color, fontFamily: 'IBM Plex Mono, monospace', fontSize: '10px', fontWeight: '700', letterSpacing: '0.08em' }}>
                    {sig.label}
                  </span>
                  <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '10px', color: 'var(--text-dim)' }}>Strength: {selectedPattern.strength}%</span>
                </div>
              </div>
              ...
              <div style={{ marginBottom: '12px' }}>
                <div style={{ height: '5px', background: 'rgba(255,255,255,0.06)', borderRadius: '3px', overflow: 'hidden' }}>
                  <div style={{ width: `${selectedPattern.strength}%`, height: '100%', background: sig.color, borderRadius: '3px', boxShadow: `0 0 8px ${sig.color}50` }} />
                </div>
              </div>

              {/* What it means */}
              <div style={{ marginBottom: '12px' }}>
                <div style={{ fontSize: '9px', color: 'var(--text-dim)', fontFamily: 'IBM Plex Mono, monospace', letterSpacing: '0.08em', marginBottom: '5px' }}>WHAT THIS MEANS</div>
                <p style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: '1.6', margin: 0 }}>{selectedPattern.meaning}</p>
              </div>

              {/* ORB Context */}
              <div style={{ marginBottom: '12px', paddingTop: '12px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                ...
                <p style={{ fontSize: '13px', color: sig.color, lineHeight: '1.6', margin: 0 }}>{selectedPattern.orbContext}</p>
              </div>

              {/* Action */}
              <div style={{ background: `${sig.color}10`, border: `1px solid ${sig.color}25`, borderRadius: '8px', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span style={{ fontSize: '16px' }}>⚡</span>
                <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '12px', color: sig.color, fontWeight: '700' }}>{selectedPattern.action}</span>
              </div>
            </div>
          )
        })() : (
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px', textAlign: 'center' }}>
            <p style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '11px', color: 'var(--text-dim)', margin: 0 }}>No pattern detected on this candle</p>
          </div>
        )
      ) : selectedIsLive ? (
        <div style={{ background: 'rgba(250,204,21,0.06)', border: '1px solid rgba(250,204,21,0.2)', borderRadius: '12px', padding: '14px 16px', textAlign: 'center' }}>
          <p style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '11px', color: '#facc15', margin: 0 }}>⏳ Live candle still forming — pattern will read on close</p>
        </div>
      ) : (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px', textAlign: 'center' }}>
          <p style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '11px', color: 'var(--text-dim)', margin: 0 }}>
            {candles.length === 0 ? 'Accumulating candle data — patterns load on candle close' : '👆 Click any completed candle to read the pattern'}
          </p>
        </div>
      )}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px' }}>
        <span style={{ fontSize: '9px', color: 'var(--text-dim)', fontFamily: 'IBM Plex Mono, monospace' }}>{candles.length} completed · {timeframe}M · MNQ · ET times</span>
        <span style={{ fontSize: '9px', color: 'var(--text-dim)', fontFamily: 'IBM Plex Mono, monospace' }}>🟡 = forming candle</span>
      </div>
    </div>
  )
}
