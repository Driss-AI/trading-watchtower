    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
      ...
      {orb && orb.breakoutDirection !== 'NONE' && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap', padding: '12px 20px', background: orb.breakoutDirection === 'LONG' ? 'rgba(93,202,165,0.12)' : 'rgba(210,90,48,0.12)', border: `1px solid ${orb.breakoutDirection === 'LONG' ? 'var(--green)' : 'var(--red)'}`, borderRadius: '10px', boxShadow: orb.breakoutDirection === 'LONG' ? '0 0 12px rgba(93,202,165,0.2)' : '0 0 12px rgba(210,90,48,0.2)' }}>
          <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '14px', fontWeight: '800', color: orb.breakoutDirection === 'LONG' ? 'var(--green)' : 'var(--red)', letterSpacing: '0.05em' }}>
      ...
      {orb && orb.breakoutDirection === 'NONE' && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px', flexWrap: 'wrap', padding: '10px 20px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '8px' }}>
          <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '10px', color: 'var(--text-dim)', fontWeight: '700', letterSpacing: '0.08em' }}>📐 OPENING RANGE</span>
          <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '12px', color: 'var(--text-secondary)' }}>High: <strong style={{ color: 'var(--green)' }}>{orb.orHigh.toFixed(2)}</strong></span>
          <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '12px', color: 'var(--text-secondary)' }}>Low: <strong style={{ color: 'var(--red)' }}>{orb.orLow.toFixed(2)}</strong></span>
          <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '12px', color: 'var(--text-secondary)' }}>Size: <strong style={{ color: 'var(--text-primary)' }}>{orb.orSize.toFixed(0)} pts</strong></span>
          <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '12px', color: 'var(--text-secondary)' }}>Now: <strong style={{ color: 'var(--text-primary)' }}>{orb.currentPrice.toFixed(2)}</strong></span>
          <span style={{ marginLeft: 'auto', fontFamily: 'IBM Plex Mono, monospace', fontSize: '10px', color: 'var(--text-dim)' }}>● watching for breakout</span>
        </div>
      )}

      {orbError && (
        <div style={{ padding: '8px 16px', background: 'var(--surface)', borderRadius: '8px', fontSize: '11px', color: 'var(--text-dim)', fontFamily: 'IBM Plex Mono, monospace' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '24px', flexWrap: 'wrap', padding: '14px 20px', background: 'var(--card)', border: `1px solid ${!pnl || pnl.net === 0 ? 'var(--border)' : pnl.net > 0 ? 'var(--green-border)' : 'var(--red-border)'}`, borderRadius: '10px' }}>
        <div>
          <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '10px', color: 'var(--text-dim)', fontWeight: '700', letterSpacing: '0.1em', marginBottom: '4px' }}>💰 DAILY P&L · LIVE</div>
          <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '28px', fontWeight: '700', color: pnlColor, lineHeight: 1 }}>
        ...
        {pnl && pnl.trades.total > 0 && (
          <>
            ...
            {pnl.fees > 0 && (
              <>
                <div style={{ height: '40px', width: '1px', background: 'var(--border)' }} />
                <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '12px', color: 'var(--text-dim)' }}>gross: ${pnl.gross.toFixed(0)}<br />fees: ${pnl.fees.toFixed(0)}</div>
              </>
            )}
          </>
        )}
        {pnl && pnl.autoCreated > 0 && (
          <>
            <div style={{ height: '40px', width: '1px', background: 'var(--border)' }} />
            <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '11px', color: 'var(--green)' }}>✓ {pnl.autoCreated} fill{pnl.autoCreated !== 1 ? 's' : ''} auto-journaled</div>
          </>
        )}
        <div style={{ marginLeft: 'auto', fontFamily: 'IBM Plex Mono, monospace', fontSize: '10px', color: 'var(--text-dim)', textAlign: 'right' }}>
