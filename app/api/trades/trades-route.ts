...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── AUTO-DETECT DIRECTION from entry/exit ──────────────────────────────
    // If exit is provided, determine direction from the prices:
    //   exit > entry → LONG (bought low, sold high)
    //   exit < entry → SHORT (sold high, bought low)
    // This prevents user entry mistakes where they swap entry/exit
    // or forget to change the direction dropdown.
    // If no exit yet (open trade), trust the user's selection.
    const direction = exit !== null
      ? (exit > entry ? 'LONG' : exit < entry ? 'SHORT' : body.direction)
      : body.direction
