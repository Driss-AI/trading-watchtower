    ...
    try {
      const res = await fetch('/api/sessions', {
        ...
        body: JSON.stringify({
          ...form,
          orSize,
          vixLevel: form.vixLevel ? parseFloat(form.vixLevel) : null,
        }),
      })
    ...
    <div>
      <div style={{ marginBottom: '28px' }}>
        ...
        <p style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '12px', color: 'var(--text-dim)', marginTop: '4px' }}>
          Enter conditions before the NY open · 9:30 AM EST
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,380px)', gap: '20px', alignItems: 'start' }}>
        ...
        <div>
          ...
          {/* Opening Range */}
          <div className="card" style={{ marginBottom: '16px' }}>
            <div style={sectionTitle}>02 · OPENING RANGE (09:30–10:00 NY)</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div>
                <label>OR High</label>
                <input
                  type="number" step="0.25" placeholder="21550.00"
                  value={form.orHigh} onChange={e => set('orHigh', e.target.value)}
                />
              </div>
              <div>
                <label>OR Low</label>
                <input
                  type="number" step="0.25" placeholder="21480.00"
                  value={form.orLow} onChange={e => set('orLow', e.target.value)}
                />
              </div>
            </div>
            ...
            {orSize !== null && (
              <div style={{
                padding: '12px 16px',
                background: orSize >= 50 && orSize <= 150 ? 'var(--green-bg)' : orSize < 30 || orSize > 200 ? 'var(--red-bg)' : 'var(--yellow-bg)',
                border: `1px solid ${orSize >= 50 && orSize <= 150 ? 'var(--green-border)' : orSize < 30 || orSize > 200 ? 'var(--red-border)' : 'var(--yellow-border)'}`,
                borderRadius: '6px',
                display: 'flex',
                alignItems: 'center',
                gap: '12px',
            ...
            <div style={{ marginTop: '12px' }}>
              <label>Trade Direction Bias</label>
          ...
          <div className="card" style={{ marginBottom: '16px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
              {[
                ...
                <label key={key} style={{
                  ...
                  textTransform: 'none', letterSpacing: 'normal', fontSize: '13px', color: 'var(--text-primary)', fontWeight: '400',
                }}>
                  <input
                    type="checkbox"
                    checked={!!(form as any)[key]}
                    onChange={e => set(key, e.target.checked)}
                    style={{ width: 'auto', accentColor: danger ? 'var(--red)' : 'var(--green)' }}
                  />
                  {label}
                </label>
              ))}
            ...
            {form.hasHighImpactNews && (
              <div style={{ marginTop: '12px' }}>
                ...
                <input type="text" placeholder="e.g. CPI at 8:30 AM, FOMC at 2:00 PM"
                  value={form.newsNotes} onChange={e => set('newsNotes', e.target.value)} />
              </div>
            )}
          </div>
          ...
          <button onClick={save} disabled={saving} className="btn btn-primary" style={{ width: '100%', padding: '14px', fontSize: '14px' }}>
            {saving ? 'Calculating...' : saved ? '✓ Session Saved' : 'Calculate Score & Save'}
          </button>
        </div>
        ...
        <div>
          {result ? (
            ...
            <div className="card" style={{ textAlign: 'center', padding: '40px 20px' }}>
              ...
              <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '13px', color: 'var(--text-dim)' }}>
                Fill in the session details and click Calculate Score
              </div>
            </div>
          )}
          ...
          <div className="card" style={{ marginTop: '16px' }}>
            ...
            {[
              ...
              'Note NQ premarket high/low',
              'Any high-impact news at 9:30 AM?',
              'VIX level — elevated or extreme?',
              'QQQ premarket direction',
              'Calculate max loss allowed today',
  ...
  return (
    <>
      <div style={{ background: bg, border: `1px solid ${border}`, borderRadius: '10px', padding: '28px', textAlign: 'center', marginBottom: '16px', boxShadow: `0 0 30px ${color}30` }}>
        <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '64px', fontWeight: '700', color, lineHeight: 1, textShadow: `0 0 20px ${color}80` }}>
          {result.score}
        </div>
        ...
        <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '16px', fontWeight: '700', color }}>
        ...
        {result.factors.map((f: ScoreFactor, i: number) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', color: f.met ? 'var(--text-primary)' : 'var(--text-dim)' }}>
              <span style={{ color: f.met ? 'var(--green)' : 'var(--text-dim)', fontSize: '14px' }}>
                {f.met ? '✓' : '✗'}
              </span>
              {f.label}
            </div>
