...
export default function Dashboard() {
  ...
  return (
    <div>
      ...
      {loading ? (
        <div style={{ color: 'var(--text-dim)', fontFamily: 'IBM Plex Mono, monospace', fontSize: '13px' }}>
          Loading...
        </div>
      ) : (
        <>
          ...
          <div style={{
            ...
            <div>
              ...
              <div style={{
                ...
                textShadow: `0 0 16px ${decisionStyle.color}80`,
              ...
              {!session && (
                <div style={{ fontSize: '13px', color: 'var(--text-secondary)', marginTop: '8px' }}>
                  Set up today's session to get your trade decision →{' '}
            ...
            <div style={{
              ...
              <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '30px', fontWeight: '700', color: decisionStyle.color, lineHeight: 1 }}>
                {score}
              </div>
            ...
            <div style={{ marginBottom: '20px', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '8px', padding: '16px 20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                ...
                <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '11px', color: 'var(--text-secondary)' }}>
                  NO TRADE &lt;65 · CAUTION 65–79 · TRADE ≥80
                </span>
              </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px', marginBottom: '20px' }}>
            <MetricCard
              label="Daily P&L"
              value={pnl >= 0 ? `+$${pnl.toFixed(0)}` : `-$${Math.abs(pnl).toFixed(0)}`}
              color={pnl > 0 ? 'green' : pnl < 0 ? 'red' : undefined}
            />
            <MetricCard
              label="Remaining Risk"
              value={remainingRisk !== null ? `$${remainingRisk.toFixed(0)}` : '—'}
              color={remainingRisk !== null && remainingRisk < settings!.dailyLossLimit * 0.3 ? 'red' : remainingRisk !== null && remainingRisk < settings!.dailyLossLimit * 0.6 ? 'yellow' : 'green'}
            />
            <MetricCard
              label="Trades Today"
              value={`${session?.tradesCount ?? 0} / ${settings?.maxTradesPerDay ?? 2}`}
              color={(session?.tradesCount ?? 0) >= (settings?.maxTradesPerDay ?? 2) ? 'red' : undefined}
            />
            <MetricCard
              label="Losses Today"
              value={`${session?.losesCount ?? 0} / ${settings?.maxLosingTradesPerDay ?? 2}`}
              color={(session?.losesCount ?? 0) >= (settings?.maxLosingTradesPerDay ?? 2) ? 'red' : undefined}
            />
          </div>
          ...
          {settings && (
            <div className="card" style={{ marginBottom: '20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
                <div>
                  <div style={{ fontSize: '10px', color: 'var(--text-secondary)', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Profit Target</div>
                  <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '18px', fontWeight: '700', color: 'var(--green)' }}>
                    ${settings.profitTarget.toLocaleString()}
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: '10px', color: 'var(--text-secondary)', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Daily Loss Limit</div>
                  <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '18px', fontWeight: '700', color: 'var(--red)' }}>
                    ${settings.dailyLossLimit.toLocaleString()}
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: '10px', color: 'var(--text-secondary)', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Trailing DD</div>
                  <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '18px', fontWeight: '700', color: 'var(--yellow)' }}>
                    ${settings.trailingDrawdown.toLocaleString()}
                  </div>
                </div>
              </div>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px' }}>
            <QuickLink href="/session" label="Setup Today's Session" icon="◈" desc="Enter OR levels & conditions" />
...
function MetricCard({ label, value, color }: { label: string; value: string; color?: string }) {
  ...
  return (
    <div className="card">
      <div style={{ fontSize: '10px', color: 'var(--text-secondary)', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: '600' }}>
        {label}
      </div>
      <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: '24px', fontWeight: '700', color: c }}>
        {value}
      </div>
    </div>
  )
